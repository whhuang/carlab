...
// Control Variables
// Error thresholds
const double ePos = 20;// 0.01; // Error threshold for position
...
void driveForward(double revs, int power) {
    ...
    if ((1 <= field_num) && (field_num <= fields)) {
      switch (field_num) {
        ...
        case 3:
          ...
          break;
        case 4: // RGB color
          cr = 28 * (num / 100);        // hundreds digit
          cg = 28 * ((num % 100) / 10); // tens digit
          cb = 28 * (num % 10);         // ones digit
