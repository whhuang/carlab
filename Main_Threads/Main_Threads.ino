...
// Packet Fields
volatile double tx; // translational error (x) in robot's reference frame
volatile double ty; // translational error (y) in robot's reference frame
volatile double r;  // rotational error in robot's reference frame
volatile double v;  // desired velocity
volatile int cr;    // red color value
volatile int cg;    // green
volatile int cb;    // blue
...
void setup() {
  ...
  threads.addThread(comThread);
  threads.addThread(driveThread);
}
